...
<dialog id="context"><button onclick="context.close()">Close</button><h2 id="compareTitle">Face comparison</h2><div id="compareItems" class="compare-items"></div><p class="muted">Blue boxes mark the selected occurrences. Yellow boxes mark other detected faces. Use the zoom controls or Ctrl/⌘ + mouse wheel; scroll the photograph to pan.</p></dialog>
<script>
...
async function comparisonItem(face,index){let item=document.createElement('article');item.className='compare-item';item.innerHTML=`<h3>Occurrence ${index+1}</h3><div class="comparison"><section class="crop-pane"><b>Face crop</b><img src="/api/faces/${face.face_id}/thumbnail"></section><section><b>Marked photograph</b><div class="zoom-controls"><button onclick="zoomBy(this,1/1.25)" aria-label="Zoom out">−</button><output class="zoom-value">100%</output><button onclick="zoomBy(this,1.25)" aria-label="Zoom in">+</button><button onclick="resetZoom(this)">Reset</button></div><div class="zoom-viewport" onwheel="wheelZoom(event)"><div class="photo-wrap" data-scale="1"><img src="/api/images/${face.image_id}/thumbnail"></div></div></section></div>`;compareItems.appendChild(item);let wrap=item.querySelector('.photo-wrap');let detections=await api('/api/images/'+face.image_id+'/faces');detections.forEach((f,i)=>{let box=document.createElement('div');box.className='face-box'+(f.face_id===face.face_id?' active':'');box.style.left=(f.x1*100)+'%';box.style.top=(f.y1*100)+'%';box.style.width=((f.x2-f.x1)*100)+'%';box.style.height=((f.y2-f.y1)*100)+'%';box.innerHTML='<span>'+(f.face_id===face.face_id?'selected':'face '+(i+1))+'</span>';wrap.appendChild(box)})}
