...
class SimpleLocalEmbeddingBackend(EmbeddingBackend):
    ...
    def embed_image(self, image_path: Path) -> np.ndarray:
        ...
        vec[:3] = arr.mean(axis=0)
        vec[3:6] = arr.std(axis=0)
...
class OpenCLIPBackend(EmbeddingBackend):
    ...
    def __init__(self, model: str | None = None, pretrained: str | None = None) -> None:
        import open_clip, torch

        self.openclip_model = model or os.getenv("KIOKUFUX_OPENCLIP_MODEL", DEFAULT_OPENCLIP_MODEL)
        self.openclip_pretrained = pretrained or os.getenv("KIOKUFUX_OPENCLIP_PRETRAINED", DEFAULT_OPENCLIP_PRETRAINED)
        self.model_version = f"{self.openclip_model}/{self.openclip_pretrained}"
        self.torch = torch
        self.model, _, self.preprocess = open_clip.create_model_and_transforms(
            self.openclip_model,
            pretrained=self.openclip_pretrained,
        )
        self.tokenizer = open_clip.get_tokenizer(self.openclip_model)
        self.model.eval()
        self.dimension = self.model.text_projection.shape[1]
...
def backend_from_options(
    ...
    if backend == "openclip":
        return OpenCLIPBackend(model=openclip_model, pretrained=openclip_pretrained)
    if backend != "auto":
...
def default_backend(openclip_model: str | None = None, openclip_pretrained: str | None = None) -> EmbeddingBackend:
    return backend_from_options("auto", openclip_model, openclip_pretrained)


def generate_embeddings(catalog: Catalog, workspace: Path, backend: EmbeddingBackend | None = None) -> int:
    backend = backend or default_backend()
    out = workspace / "embeddings"
    out.mkdir(parents=True, exist_ok=True)
    count = 0
    for photo in catalog.list_photos():
        target = out / f"{photo.photo_id}.{backend.model_name}.npy"
        if target.exists() and photo.embedding_status == "indexed":
            continue
        try:
            ...
            catalog.conn.execute(
                "UPDATE photos SET embedding_status='error', error=? WHERE photo_id=?",
                (f"embedding: {exc}", photo.photo_id),
            )
            catalog.conn.commit()
    return count
