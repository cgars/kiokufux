...
async function confirmPerson(){if(!currentGroup)return notify('Open a reviewed group first.','error');let name=prompt('Optional display name (leave blank to remain unnamed):','');if(name===null)return;try{await withFeedback('Person confirmation',()=>mutate('/api/people',{group_id:currentGroup.group_id,display_name:name||null}));await showGroups()}catch(e){}}
...
def _confirmed_people(store: FaceStore, state: ReviewState) -> list[dict[str, object]]:
    people = {person["person_id"]: dict(person) for person in state.people.get("people", [])}
    rows = []
    for person_id, face_ids in sorted((state.review.get("person_faces") or {}).items()):
        person = people.get(person_id, {"person_id": person_id, "friendly_name": person_id, "display_name": None})
        unique_face_ids = list(dict.fromkeys(face_ids))
        if unique_face_ids:
            placeholders = ",".join("?" for _ in unique_face_ids)
            face_rows = store.db.execute(f"""SELECT face_id,image_id FROM face_occurrences
              WHERE face_id IN ({placeholders}) ORDER BY face_id""", unique_face_ids).fetchall()
        else:
            face_rows = []
        rows.append({
            "person_id": person_id,
