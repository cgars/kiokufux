...
<dialog id="context"><button onclick="context.close()">Close</button><img id="contextImage"><p id="contextCaption"></p></dialog>
<script>
...
function faceCard(f){return `<article class="face" data-id="${f.face_id}" onclick="toggleFace(event,this)"><input type="checkbox"><img src="/api/faces/${f.face_id}/thumbnail"><button onclick="viewContext(event,'${f.image_id}')">View photograph</button><small>confidence ${(f.confidence*100).toFixed(1)}%</small></article>`}
...
function viewContext(e,id){e.stopPropagation();contextImage.src='/api/images/'+id+'/thumbnail';contextCaption.textContent='Complete photograph containing this occurrence';context.showModal()}
